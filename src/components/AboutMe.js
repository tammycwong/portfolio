import React from 'react';


function AboutMe() {

    return (
    <div className="">
      <h2>About Me</h2>
      <div className="aboutme-container">
      <p className="aboutme">
      A career changer from hospitality management to tech.<br/>
      I have a serious passion for creating fun and beneficial applications;<br/>
      building an engaging user experience and learning the best way to do so, i find, is the most exhilarating journey.<br/>
      {/* One of my strengths is frontend engineering as well as conceptual thinking. <br/> */}

      I first started coding when I was 15, personalizing my layouts for my blog platforms- Xanga and LiveJournal. <br/>
      I loved changing themes to my site through custom banners, mp3s and dynamic styling with JavaScript and CSS. <br/>

      My background in cognitive and behaviorial psychology has helped me understand the human mind and how it works with user experience design.<br/>

      </p>
      </div>
    </div>
    
    )
}

export default AboutMe;